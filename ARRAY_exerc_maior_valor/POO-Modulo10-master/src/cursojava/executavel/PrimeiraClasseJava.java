...
public class PrimeiraClasseJava {
	...
	public static void main(String[] args) {
		...
		if (new FuncaoAutenticacao(new Diretor(login, senha)).autenticar()) {/* Acesso para diretor se for TRUE acessa , se for FALSE não acessa*/
			
		//if (new FuncaoAutenticacao(new Secretario(login, senha)).autenticar()) {/*acesso para secretario se for TRUE acessa , se for FALSE não acessa*/
			
			JOptionPane.showMessageDialog(null, "Acesso liberado ao sistema! Pressione OK!");
			
			List<Aluno> alunos = new ArrayList<Aluno>();		
			
			// Estrutura de dados que representa os elementos por uma chave e um valor associado à chave
		...
		}catch (NumberFormatException e) {// captura qualquer excessão do formato do numero do programa
			// TODO: handle exception
			StringBuilder saida = new StringBuilder();// trabalhando com texto
			
			e.printStackTrace();// imprime o erro no console java
			System.out.println(" mesnsagens : +" + e.getMessage()); // mensagem do erro ou causa
			...
			JOptionPane.showMessageDialog(null, "Houve um erro de conversão de numero " + saida.toString());
		}catch (NullPointerException e) {// captura qualquer excessão do tipo ponto nulo do programa
			// TODO: handle exception
			JOptionPane.showMessageDialog(null, "erro de NullPointerExcepiton " +e.getClass());
		}catch (Exception e) {// captura qualquer excessão do programa
			// TODO: handle exception
			e.printStackTrace();
		}finally { /* sempre é executado ocorrendo erros ou não
		o finaly sempre é usado quando precisa executar um processo acontecendo erro ou não no sistema */
			JOptionPane.showMessageDialog(null, "voce esta apredendo java ");
		}
	}	
}
